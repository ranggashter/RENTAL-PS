...
class Welcome extends CI_Controller
{
	...
	public function start()
	{
		...
		if ($waktu == "M") { // New case for 30 Menit
			$wktu = date('Y-m-d H:i:s', time() + (60 * 30)); // 30 menit kedepan
			$paket = "PLAYSTATION 3 main : 30 Menit"; // mAIN 30 Menit
	    } else if ($waktu == "A") {
			$wktu = date('Y-m-d H:i:s', time() + (60 * 60)); //1 jam kedepan
			$paket = "PLAYSTATION 3 main  : 1 Jam";
		} else if ($waktu == "C") {
			$wktu = date('Y-m-d H:i:s', time() + (60 * 60) * 2); //2 jam kedepan
			$paket = "PLAYSTATION 3 main  : 2 Jam";
		} else if ($waktu == "E") {
			$wktu = date('Y-m-d H:i:s', time() + (60 * 60) * 3); //3 jam kedepan
			$paket = "PLAYSTATION 3 main : 3 Jam";
		} else if ($waktu == "G") {
			$wktu = date('Y-m-d H:i:s', time() + (60 * 60) * 4); //4 jam kedepan
			$paket = "PLAYSTATION 3 main : 4 Jam";
		} else if ($waktu == "L") { // New case for 5 Jam
			$wktu = date('Y-m-d H:i:s', time() + (60 * 60) * 5); // 5 jam kedepan
			$paket = "PLAYSTATION 3 main : 5 Jam"; // Paket 5 Jam
		} else if ($waktu == "AA") {
			$wktu = date('Y-m-d H:i:s', time() + (60 * 60)); //1 jam kedepan
			$paket = "PLAYSTATION 3 main diskon : 1 Jam";
		} else if ($waktu == "CC") { // Main Diskon 2 Jam
			$wktu = date('Y-m-d H:i:s', time() + (60 * 60) * 2); // 2 jam kedepan
			$paket = "PLAYSTATION 3 main diskon : 2 Jam";
		} else if ($waktu == "EE") {
			$wktu = date('Y-m-d H:i:s', time() + (60 * 60) * 3); //3 jam kedepan
			$paket = "PLAYSTATION 3 main diskon: 3 Jam";
		}  else if ($waktu == "GG") {
			$wktu = date('Y-m-d H:i:s', time() + (60 * 60) * 4); //4 jam kedepan
			$paket = "PLAYSTATION 3 main diskon : 4 Jam";
		} else if ($waktu == "LL") { // New case for 5 Jam
			$wktu = date('Y-m-d H:i:s', time() + (60 * 60) * 5); // 5 jam kedepan
			$paket = "PLAYSTATION 3 main : 5 Jam"; // Paket 5 Jam
		} else if ($waktu == "K") { // New case for Diskon 3 Jam + 1 Jam
			$wktu = date('Y-m-d H:i:s', time() + (60 * 60) * 4); // 4 jam kedepan
			$paket = "PLAYSTATION 3 main paket : 3 Jam + 1 Jam"; // Changed to Diskon 3 Jam + 1 Jam
		} else if ($waktu == "") { // Check if $waktu is empty
			$wktu = "";
			$paket = "Paket : 0 Jam";
